...
func RunPrometheusListenerForNginx(port string, client *prometheusClient.NginxClient) {
	registry := prometheus.NewRegistry()
	registry.MustRegister(collector.NewNginxCollector(client, "nginx"))
	runServer(port, registry)
}

// RunPrometheusListenerForNginxPlus runs an http server to expose Prometheus metrics for NGINX Plus
func RunPrometheusListenerForNginxPlus(port string, plusClient *sdkClient.NginxClient) {
	registry := prometheus.NewRegistry()
